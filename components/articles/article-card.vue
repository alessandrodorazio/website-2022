<script setup>
const props = defineProps([
    'article'
])
//const key = props.article.Key
//const title = (props.article?.meta?.name) ? props.article.meta.name : props.article.Key
//const excerpt = props.article.meta?.excerpt?.replace(/<br>/ig, ' ').replace(/(<([^>]+)>)/ig, '').replace(/(<|>)/ig, '').replaceAll(/\&amp;nbsp;/ig, ' ')

//const link = key.replace('blog/', '').replace('.json', '')
//const isStarred = props.article.meta?.star === '1'
const dateFormatted = props.article.date.toLocaleDateString()
</script>

<template>
    <div :class="{'article-card-container-not-starred-bg-gradient': !props.article.starred}">
        <div class="article-card-container" :class="{'article-card-container-starred': props.article.starred, 'article-card-container-not-starred': !props.article.starred}">
            <div class="article-card-link-container">
                <NuxtLink :href="'/blog/' + props.article.slug + '.html'" :class="{'mb-1' : !props.article.starred}">
                    <h2 class="article-card-link-title-container">
                                    <span v-if="props.article.starred">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" class="w-5 mr-1"><!-- Font Awesome Pro 5.15.4 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) --><defs></defs><path
                                                d="M528.53 171.5l-146.36-21.3-65.43-132.39c-11.71-23.59-45.68-23.89-57.48 0L193.83 150.2 47.47 171.5c-26.27 3.79-36.79 36.08-17.75 54.58l105.91 103-25 145.49c-4.52 26.3 23.22 46 46.48 33.69L288 439.56l130.93 68.69c23.26 12.21 51-7.39 46.48-33.69l-25-145.49 105.91-103c19-18.49 8.48-50.78-17.79-54.57zm-90.89 71l-66.05 64.23 15.63 90.86a12 12 0 0 1-17.4 12.66L288 367.27l-81.82 42.94a12 12 0 0 1-17.4-12.66l15.63-90.86-66-64.23A12 12 0 0 1 145 222l91.34-13.28 40.9-82.81a12 12 0 0 1 21.52 0l40.9 82.81L431 222a12 12 0 0 1 6.64 20.46z"
                                                class="fill-yellow-500"/><path
                                                d="M437.64 242.46l-66.05 64.23 15.63 90.86a12 12 0 0 1-17.4 12.66L288 367.27l-81.82 42.94a12 12 0 0 1-17.4-12.66l15.63-90.86-66-64.23A12 12 0 0 1 145 222l91.34-13.28 40.9-82.81a12 12 0 0 1 21.52 0l40.9 82.81L431 222a12 12 0 0 1 6.64 20.46z"
                                                class="fill-yellow-500"/></svg>
                                    </span>
                        <span>
                        <span v-text="props.article.title" class="article-card-link-title-content"
                              :class="{
                        'from-orange-600 to-red-600': props.article.starred,
                        'from-blue-800 to-purple-800': !props.article.starred
                    }"></span>
                    <span v-if="props.article.starred" v-text="'Last update: ' + dateFormatted"
                          class="article-card-updated-at-content"></span>
                    </span>


                    </h2>
                </NuxtLink>
            </div>

            <p class="article-card-updated-at-content" v-if="! props.article.starred ">
                <svg xmlns="http://www.w3.org/2000/svg"
                     viewBox="0 0 448 512" class="h-3 fill-gray-500"><!-- Font Awesome Pro 5.15.4 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) -->
                    <path d="M12 192h424c6.6 0 12 5.4 12 12v260c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V204c0-6.6 5.4-12 12-12zm436-44v-36c0-26.5-21.5-48-48-48h-48V12c0-6.6-5.4-12-12-12h-40c-6.6 0-12 5.4-12 12v52H160V12c0-6.6-5.4-12-12-12h-40c-6.6 0-12 5.4-12 12v52H48C21.5 64 0 85.5 0 112v36c0 6.6 5.4 12 12 12h424c6.6 0 12-5.4 12-12z"/>
                </svg>
                <span v-text="dateFormatted"></span>
            </p>

            <p v-if="props.article.excerpt && !props.article.starred"
               v-text="props.article.excerpt"
               class="article-card-excerpt"></p>

        </div>
    </div>
</template>

<style scoped>

</style>